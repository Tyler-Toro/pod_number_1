...
available_books()
# SInce it already has the print function within it, I dont need to add it for it to print
...
def check_out(lst):
    ...
    print(books.remove(lst))


check_out('MY OWN WORDS')
...
def search_by_name(lst):
    # for title in books:
        if lst in books:
            print(F"{lst} is available")
        
        elif lst not in books:
                print(f"{lst} not available")
...
search_by_name("f")
# GOT IT !!!111!
# search_by_name('THE COLOR OF LAW')
...
# def search_by_author(string):
#     authors_books = []
#     for string in books_with_details:
#         if string ['author'] == books_with_details:
#             authors_books.append(books_with_details['title'])
