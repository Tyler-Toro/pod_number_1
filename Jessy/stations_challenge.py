...
#  I put the parent inside the parathensis so the child can inherant its attributes.

class SubwayStation(Station):
    def __init__(self, station_name, location, lines):
# used super() so I wont need to type self again
        super().__init__(station_name, location)
        self.lines = lines
# I instantiated the information for my s1
# s1= SubwayStation('14th street', '14th street and 7th avenue', ['1','2', '3', 'L'])
    def show_info(self):
        print(f'{self.station_name} station is located at {self.location} and has lines:{self.lines}')
s1= SubwayStation('14th street', '14th street and 7th avenue', ['1','2', '3', 'L'])
s1.show_info()
...
class BusStation(Station):
    def __init__(self, station_name, location, routes= [], open = True):
        # used super() so I wont have to write 'self'
        super().__init__(station_name, location)
        # made additional attributes that as of now are only unique to this child class
        self.routes = routes
        self.open = open
...
# overrode the method in order to show if the bus route is open
    def show_info(self):
        # if self.open== True:
        #     return 'Open'
        # else: return 'Off'

        print(f"{self.station_name} station is located at {self.location} it's routes are {self.routes}")
        # ' and currently they are {self.open}")
        if self.open == False:
            print('Station is open.')
        else:
            print('Station is closed.')
...
'''
bs1 = BusStation('NYC Megabus Stop','34th street and 12th avenue', ['Boston', 'DC', 'Philly'],open=True)
# print(bs1.show_info())
bs1.open_station()
bs1.close_station()
bs1.show_info()
