from pprint import pprint #In our pod, someone shared that his helps to make the output of our code look more organized (when you print a statement add an extra p before it)
...
def available_books(): #can put argument in the parenthesis to add an argument...blank has no specificity
    print(books)
...
def check_out(lst):
    books.remove(lst) #using this to remove..(POP IS REMOVE IN DICTIONARY)


# 1.4
# Check out 'SAPIENS' using the check_out function
check_out('SAPIENS')
print(books) #old way. The better way is using the functiob you creating (available_books). This prints books already
...
def check_in(lst):
    books.append(lst) #we are using append because we are adding the book back
...
print(type(books_with_details[0])) #This indexes what is in the list and explains the type. They typs is the same for all items in the list
...
def count_books():
    ...
    return #return is used to stop the function
